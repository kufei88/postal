...
layui.define(['element', 'common', 'paging', 'form'], function (exports) {
    ...
    BTable.prototype.render = function () {
        ...
        var table_content = '';
        console.log($('.btable-content2'))
        if($('.btable-content').length > 0){
        	 table_content = 'btable-content1';
        }
        else{
        	table_content = 'btable-content';
        }
        tpl += '<tbody class="'+table_content+'"></tbody>';
        tpl += '</table>';
        if (_config.paged) {
        ...
        paging.init({
            url: _config.url, //地址
            elem: _config.elem+' .'+table_content, //内容容器
            type: _config.type,
